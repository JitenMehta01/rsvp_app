 ...
 *  - A INPUT ELEMENT
 * 
 *  2. ADD THE TEXT ' Hide those who haven't responded ' TO THE LABEL AND DETERMINE THE INPUT TYPE.
 *  3. APPEND THE LABEL AND CHECKBOX TO THE DIV, AND THEN APPEND THE DIV BEFORE THE UL VARIABLE TO THE MAINDIV VARIABLE. 
 * 
 */
 ...
 * 
 *
 *  CREATE A CHANGE EVENT WHICH LISTENS FOR THE CHECKBOX. 
 *  
 ...
 *  -   THEN CREATE  A IF STATEMENT. CHECK WHETHER IF THE LIST ITEMS CLASSNAME IS 'responded'. 
 *  -   IF TRUE, THEN SET ITS DISPLAY PROPERTY TO INITIAL. IF FALSE THEN SET ITS DISPLAY PROPERTY TO NONE.
 *   
 *  3. IF THE IF STATEMENT IS FALSE, LOOP THROUGH ALL LIST ITEMS AND SET ITS DISPLAY PROPERTY TO INITIAL.
 * 
 *  - A LABEL ELEMENT
 *  - A INPUT ELEMENT
 * 
...
filtercheckBox.addEventListener('change', (e) =>{
  isChecked = filtercheckBox.checked;
  let list = ul.children;

  if (isChecked){
    for (let i =0; i < list.length; i++){
      let item = list[i];
      if(item.className === 'responded'){
        item.style.display = ''
      } else{
 ...
 *  -  THEN APPEND THE CHECKBOX TO THE LABEL, AND THEN APPEND THE LABEL TO THE LIST ITEM.
 * 
 ...
 * 
 *  1. PREVENT THE DEFAULT BEHAVIOUR OF A FORM, WHICH REFRESHES WHENEVER THE SUBMIT BUTTON IS CLICKED.
 *  2. STORE THE VALUE OF THE INPUT VARIABLE INTO ANOTHER VARIABLE CALLED TEXT.
 *  3. THEN CREATE A ANOTHER VARIAVBLE AND STORE THE CREATELI FUNCTION WITH THE INPUT VALUE VARIABLE PASSED THROUGH AS AN ARGUEMENT 
 ...
 *  
 *  6. THE SECONED BRANCH WILL CHECK IF THE BUTTON VARIABLE'S TEXT CONTENT IS 'edit'.
 ...
 */
  ...
  ul.addEventListener('click', (e) =>{
    if(e.target.tagName === 'BUTTON'){
      ...
      if(button.textContent === 'responded'){
        ul.removeChild(li);
